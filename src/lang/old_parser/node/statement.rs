...
use crate::{
    lang::{
        token::Token,
        old_parser::exception::{ExpectedToken, ParserError, ParserException}
    },
    vm::context::Context
};
...
impl NodeFactory for Statement {
    ...
    fn from_tokens(m: &mut crate::lang::old_parser::machine::ParsingMachine) -> Result<Self, ParserError> where Self: Sized {
        match m.peek() {
